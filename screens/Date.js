...
export default class DateScreen extends Component {
  constructor(props){
    super(props)
    this.state = {data:null,focus:'startDate',startDate:null,enddDate:null,isModalVisible:'false',userid:firebase.auth().currentUser.email,getnoofdays:''}
  }
  ...
  showmodal = () => {
    return(
      ...
      >
        ...
        <TouchableOpacity style={[styles.smallbutton,{marginLeft:20}]}>
          <Text>Cancel</Text>
        </TouchableOpacity>
        </View>
      </Modal>
  ...
  render(){
    return (
      ...
      <Text> {this.state.getnoofdays} </Text>
